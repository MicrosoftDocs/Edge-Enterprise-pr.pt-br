...
Por padrão, quando o Microsoft Edge é implantado no Windows 10, os dados de diagnóstico são enviados com base na [Configuração de dados de diagnóstico do Windows](/windows/privacy/configure-windows-diagnostic-data-in-your-organization) dos usuários.
...
- (PRETERIDA) [MetricsReportingEnabled](./microsoft-edge-policies.md#metricsreportingenabled) - Habilita a geração de relatórios de dados relacionados a falhas e de uso. Essa política se tornará obsoleta na versão 89 do Microsoft Edge.
- (PRETERIDA) [SendSiteInfoToImproveServices](./microsoft-edge-policies.md#sendsiteinfotoimproveservices) - Envia informações do site para melhorar os serviços Microsoft. Essa política se tornará obsoleta na versão 89 do Microsoft Edge.

As políticas preteridas mencionadas são substituídas por [Permitir Telemetria](/windows/privacy/configure-windows-diagnostic-data-in-your-organization) no Windows 10 e a política [DiagnosticData](./microsoft-edge-policies.md#diagnosticdata) para todas as outras políticas.  
...
Se a política [DiagnosticData](./microsoft-edge-policies.md#diagnosticdata) estiver configurada, ela tem precedência sobre [MetricsReportingEnabled](./microsoft-edge-policies.md#metricsreportingenabled)  e [SendSiteInfoToImproveServices](./microsoft-edge-policies.md#sendsiteinfotoimproveservices).

#### <a name="required-and-optional-diagnostic-data"></a>Dados de diagnóstico obrigatórios e opcionais
...
> [!NOTE]
> Essa política não é compatível com dispositivos com Windows 10. Para controlar a coleta de dados no Windows 10, os administradores de TI devem usar a política de grupo de dados de diagnóstico do Windows. Essa política irá **Permitir Telemetria** ou **Permitir Dados de Diagnóstico**, dependendo da versão do Windows. Saiba mais sobre a [coleta de dados de diagnóstico do Windows 10](/windows/privacy/configure-windows-diagnostic-data-in-your-organization).
...
### <a name="deprecated-enable-usage-and-crash-related-data-reporting"></a>(PRETERIDA) Habilitar a geração de relatórios de dados relacionados a falhas e de uso
...
- Se essa política estiver habilitada, o Microsoft Edge enviará dados de uso somente se a configuração de dados de diagnóstico do Windows estiver definida como **Avançada** ou **Completa**.
  - Se essa política estiver habilitada, o Microsoft Edge só enviará dados de uso se [SendSiteInfoToImproveServices](./microsoft-edge-policies.md#sendsiteinfotoimproveservices) também estiver habilitada.
- Se essa política estiver desabilitada, o Microsoft Edge não enviará dados de uso. Os dados relacionados a falha são enviados com base na configuração de dados de diagnóstico do Windows. [Saiba mais sobre as configurações de Dados de diagnóstico do Windows:](/windows/privacy/configure-windows-diagnostic-data-in-your-organization).
...
### <a name="deprecated-send-site-information-to-improve-microsoft-services"></a>(PRETERIDA) Enviar informações de sites para aprimorar os serviços Microsoft
...
- Se essa política estiver habilitada, o Microsoft Edge enviará informações dos sites visitados apenas se a configuração de dados de diagnóstico do Windows estiver definida como **Completa**.
  - Se essa política estiver habilitada, o Microsoft Edge só enviará dados de uso se [MetricsReportingEnabled](./microsoft-edge-policies.md#metricsreportingenabled) também estiver habilitada. 
- Se essa política estiver desabilitada, o Microsoft Edge não enviará informações dos sites visitados. Para [saber mais sobre as configurações de dados de diagnóstico do Windows](/windows/privacy/configure-windows-diagnostic-data-in-your-organization).
...
- Se a política [DiagnosticData](./microsoft-edge-policies.md#diagnosticdata) estiver configurada, ela terá precedência sobre [MetricsReportingEnabled](./microsoft-edge-policies.md#metricsreportingenabled)  e [SendSiteInfoToImproveServices](./microsoft-edge-policies.md#sendsiteinfotoimproveservices). 
- Se a política [DiagnosticData](./microsoft-edge-policies.md#diagnosticdata) não estiver configurada, o Microsoft Edge escutará [MetricsReportingEnabled](./microsoft-edge-policies.md#metricsreportingenabled)  e [SendSiteInfoToImproveServices](./microsoft-edge-policies.md#sendsiteinfotoimproveservices).  
...
## <a name="additional-privacy-policy-options"></a>Opções adicionais da política de privacidade
...
- [White paper de Privacidade do Microsoft Edge](/microsoft-edge/privacy-whitepaper)